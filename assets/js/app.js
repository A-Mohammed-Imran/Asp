...
function initDashboardPage() {
	...
	const isDashboard = document.querySelector('.sidebar .nav-link.active')?.textContent?.trim() === 'Dashboard'
	...
	const recentActivities = [
		...
		{ icon: '🎓', action: 'Completed Advanced CSS Module', time: '5 hours ago' },
		{ icon: '📅', action: 'Interview scheduled with TechFlow', time: 'Yesterday' },
		{ icon: '🏆', action: 'Earned "Early Bird" achievement', time: '2 days ago' }
	];

	function renderStats() {
		const container = document.getElementById('stats-container');
		if (!container) return; // container not present in current markup
		container.innerHTML = dashboardStats.map(stat => `
			<div class="card">
				<span class="stat-label">${stat.label}</span>
				<span class="stat-value" style="color: ${stat.color}">${stat.value}</span>
				<div style="height: 4px; width: 100%; background: rgba(255,255,255,0.05); border-radius: 2px;">
					<div style="height: 100%; width: ${stat.value.includes('%') ? stat.value : '70%'}; background: ${stat.color}; border-radius: 2px;"></div>
				</div>
			</div>
		`).join('');
	}
...
function initDailyGoalsPage() {
	...
	if (!goalsContainer || !goalForm || !progressCircle || !progressPercent || !summaryText) return; // Not on goals page

	let goals = JSON.parse(localStorage.getItem('asprasys_goals')) || [
		{ id: 1, title: 'Morning Stand-up', desc: 'Sync with the engineering team', completed: true },
		{ id: 2, title: 'Code Review', desc: 'Review PRs for the authentication module', completed: false }
	];
	...
	function updateProgressUI() {
		...
		summaryText.innerText = remaining === 0
			? 'Excellent! All goals for today are completed.'
			: `You have ${remaining} goal${remaining > 1 ? 's' : ''} remaining for today.`;
	}
	...
	function renderGoals() {
		...
		goals.forEach(goal => {
			...
			goalEl.innerHTML = `
				...
				<span class="goal-status-badge ${goal.completed ? 'badge-completed' : 'badge-pending'}">
